...
import { ArrowLeft, Share2, Bookmark, Clock, Users, BarChart2, Minus, Plus, CheckSquare, Square, ChevronLeft, ChevronRight, X, Timer } from 'lucide-react';
import StarRating from '../components/StarRating';
import NutritionBadge from '../components/NutritionBadge';
import Toast from '../components/Toast';
import { allRecipes, sampleReviews } from '../data/mockData';

const TABS = ['Ingredients', 'Instructions', 'Nutrition', 'Reviews'];

function getCuisineEmoji(c) {
  const m = { italian:'🍝', mexican:'🌮', asian:'🍜', mediterranean:'🫒', indian:'🍛', american:'🍔', french:'🥐', japanese:'🍱', 'middle-eastern':'🥙', pakistani:'🫓' };
  return m[c] || '🍽️';
}
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', padding: '20px 16px 0', flexShrink: 0 }}>
        ...
        <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--color-text-3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
        <div style={{
          ...
          <p style={{ fontSize: 'var(--text-lg)', color: 'var(--color-text)', lineHeight: 1.65, fontFamily: 'var(--font-display)' }}>
            {step?.text}
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Timer size={20} color={running ? 'var(--color-primary)' : 'var(--color-text-3)'} />
            <div>
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-3)', fontWeight: 600 }}>Step Timer</p>
              <p style={{ fontSize: 28, fontWeight: 700, fontFamily: 'monospace', color: running ? 'var(--color-primary)' : 'var(--color-text)', lineHeight: 1 }}>
                {formatTime(timeLeft)}
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              style={{ padding: '10px 14px', borderRadius: 'var(--radius-full)', background: 'var(--color-surface)', border: '1px solid var(--color-border)', cursor: 'pointer', fontWeight: 600, fontSize: 'var(--text-sm)', color: 'var(--color-text-2)', fontFamily: 'var(--font-body)' }}
            >
              Reset
            </button>
          </div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text-2)',
            fontFamily: 'var(--font-body)',
          }}
        >
...
export default function RecipeDetail() {
  ...
  const recipe = allRecipes.find(r => r.id === Number(id))
    || (() => { try { const ai = JSON.parse(localStorage.getItem('AI_RECOMMENDATIONS') || '[]'); return ai.find(r => r.id === Number(id)); } catch { return null; } })()
    || allRecipes[0];
  ...
  function handleSave() {
    setSaved(s => !s);
    setToast(saved ? 'Removed from saved' : 'Recipe saved! ❤️');
  }
  ...
  return (
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh', paddingBottom: 100 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          ...
          <button
            className="btn-icon"
            onClick={handleSave}
            style={{ background: saved ? 'var(--color-primary-light)' : undefined }}
          >
            <Bookmark size={16} style={{ fill: saved ? 'var(--color-primary)' : 'transparent', color: saved ? 'var(--color-primary)' : 'var(--color-text-2)' }} />
          </button>
        </div>
      </div>
      ...
      <div style={{ height: 260, position: 'relative', background: recipe.gradient, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 16px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
            {[
              { icon: '⏱', label: `${recipe.time} min` },
              { icon: '👤', label: `${recipe.servings} servings` },
              { icon: '📊', label: recipe.difficulty },
            ].map(m => (
              <span key={m.label} style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.92)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 3 }}>
                {m.icon} {m.label}
              </span>
            ))}
          </div>
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
          <button className="btn btn-primary btn-sm" onClick={handleSave}>
            <Bookmark size={14} /> {saved ? 'Saved' : 'Save Recipe'}
          </button>
          <button className="btn btn-secondary btn-sm" onClick={() => navigate('/meal-planner')}>
            📅 Add to Plan
          </button>
        ...
        {tab === 'Ingredients' && (
          <div>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-text)' }}>Adjust servings</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <button onClick={() => setServings(s => Math.max(1, s - 1))} style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--color-surface)', border: '1px solid var(--color-border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Minus size={14} />
                </button>
                <span style={{ fontSize: 'var(--text-md)', fontWeight: 700, minWidth: 20, textAlign: 'center' }}>{servings}</span>
                <button onClick={() => setServings(s => Math.min(12, s + 1))} style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--color-primary)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                  <Plus size={14} />
                </button>
              </div>
            </div>
            ...
            {recipe.ingredients.map((ing, i) => (
              ...
              >
                ...
                <span style={{ fontSize: 'var(--text-base)', color: 'var(--color-text)', flex: 1, textDecoration: checked[i] ? 'line-through' : 'none' }}>
                  {ing.qty && `${(parseFloat(ing.qty) * scale || ing.qty)} ${ing.unit} `}{ing.name}
                </span>
              </button>
            ))}
          </div>
            ...
            {recipe.steps.map((s, i) => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <span className="badge badge-surface"><Clock size={10} /> {s.time}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {tab === 'Reviews' && (
          <div>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24, padding: '16px', background: 'var(--color-surface)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)' }}>
              ...
              <p style={{ color: 'var(--color-text-3)', fontSize: 'var(--text-sm)' }}>{recipe.reviewCount} reviews</p>
              {/* Bar breakdown */}
              {[5,4,3,2,1].map(star => (
                <div key={star} style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                  ...
                  <div style={{ flex: 1, height: 6, background: 'var(--color-surface-2)', borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${star === 5 ? 70 : star === 4 ? 20 : 5}%`, background: 'var(--color-warning)', borderRadius: 3, transition: 'width 0.5s' }} />
                  </div>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-3)', width: 24 }}>{star === 5 ? '70%' : star === 4 ? '20%' : '5%'}</span>
                </div>
              ))}
            </div>

            {/* Rate it yourself */}
            <div style={{ background: 'var(--color-surface-2)', borderRadius: 'var(--radius-md)', padding: '14px 16px', marginBottom: 20, border: '1px solid var(--color-border)', textAlign: 'center' }}>
              <p style={{ fontSize: 'var(--text-sm)', fontWeight: 600, marginBottom: 10 }}>Rate this recipe</p>
            ...
            {sampleReviews.map(rev => (
              <div key={rev.id} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '14px 16px', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--color-primary)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 'var(--text-sm)', flexShrink: 0 }}>
                    {rev.initial}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontWeight: 600, fontSize: 'var(--text-sm)' }}>{rev.name}</span>
                      <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-3)' }}>{rev.date}</span>
                    </div>
                    <StarRating rating={rev.rating} size={12} />
                  </div>
                </div>
                <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-2)', lineHeight: 1.6 }}>{rev.comment}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{
        ...
        <button className="btn btn-primary btn-lg" style={{ width: '100%' }} onClick={() => setCookingMode(true)}>
          🍳 Start Cooking
        </button>
      </div>
