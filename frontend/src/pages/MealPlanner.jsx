...
import { X, Plus, Zap, Search as SearchIcon } from 'lucide-react';
import { weekPlan as initialPlan, allRecipes } from '../data/mockData';
...
export default function MealPlanner() {
  ...
  function addRecipe(day, meal, recipe) {
    setPlan(p => ({ ...p, [day]: { ...p[day], [meal]: recipe } }));
    setShowPicker(null);
  ...
  function generateWeek() {
    const newPlan = {};
    DAYS.forEach((day, di) => {
      const breakfastRecipes = allRecipes.filter(r => r.category === 'breakfast');
      const lunchRecipes = allRecipes.filter(r => r.category === 'lunch');
      const dinnerRecipes = allRecipes.filter(r => r.category === 'dinner');
      newPlan[day] = {
        breakfast: breakfastRecipes[di % breakfastRecipes.length] || allRecipes[1],
        lunch:     lunchRecipes[di % lunchRecipes.length]         || allRecipes[2],
        dinner:    dinnerRecipes[di % dinnerRecipes.length]       || allRecipes[0],
      };
    });
    setPlan(newPlan);
  ...
  // Picker filtering — all recipes, search + category
          ...
          return (
            <div key={meal} style={{ marginBottom: 14 }}>
              ...
              {recipe ? (
                ...
                >
                  <div style={{
                    ...
                    {recipe.image
                      ...
                      : <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>{getCuisineEmoji(recipe.cuisine)}</span>
                    }
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-3)', marginTop: 2 }}>
                      {recipe.time} min · {recipe.calories} kcal · ⭐ {recipe.rating}
                    </p>
                  </div>
                  <button
                ...
                <button
                  onClick={() => setShowPicker({ day: activeDay, meal })}
                  style={{
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexShrink: 0 }}>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-lg)', fontWeight: 600 }}>
                {MEAL_ICONS[showPicker.meal]} Add {MEAL_LABELS[showPicker.meal]}
              </h3>
              <button
                onClick={() => setShowPicker(null)}
            ...
            <div style={{ position: 'relative', marginBottom: 10, flexShrink: 0 }}>
              ...
              <input
                ...
                style={{ borderRadius: 'var(--radius-full)', fontSize: 'var(--text-sm)', paddingRight: pickerSearch ? 36 : 12 }}
              />
              {pickerSearch && (
                <button
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        {r.image
                          ...
                          : <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>{getCuisineEmoji(r.cuisine)}</span>
                        }
                      </div>
