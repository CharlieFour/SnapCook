...
export default function EditProfile() {
  ...
  const [form, setForm] = useState({
    ...
    phone: '',
    bio: 'Home cook & food enthusiast. Love exploring Pakistani and Italian cuisines.',
    location: 'London, UK',
    website: '',
  });
  ...
  function Field({ label, field, placeholder, multiline, keyboardType }) {
    return (
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh', paddingBottom: 40 }}>
      ...
      <div style={{ padding: '24px 16px 0' }}>
        ...
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '20px 16px', marginBottom: 16, boxShadow: 'var(--shadow-sm)' }}>
          ...
          <Field label="Username" field="username" placeholder="e.g. rafaycooks" />
          <Field label="Bio" field="bio" placeholder="Tell us about yourself…" multiline />
          <Field label="Location" field="location" placeholder="City, Country" />
          <Field label="Website" field="website" placeholder="https://" />
        </div>

        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '20px 16px', marginBottom: 16, boxShadow: 'var(--shadow-sm)' }}>
          <p style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--color-text-3)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 16 }}>Contact</p>
          <Field label="Email" field="email" placeholder="you@example.com" keyboardType="email" />
          <Field label="Phone" field="phone" placeholder="+44 7700 900000" keyboardType="tel" />
        </div>
